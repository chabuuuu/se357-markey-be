...
class CartItemContainer extends BaseContainer {
  constructor() {
    super(CartItem);
    this.container.bind<ICartItemService<ICartItem>>('CartItemService').to(CartItemService);
    this.container.bind<ICartItemRepository<ICartItem>>('CartItemRepository').to(CartItemRepository);
  }
